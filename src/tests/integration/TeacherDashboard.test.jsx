...
describe('TeacherDashboard - UI y Modal de estudiantes', () => {
  ...
  it('abre modal de estudiantes al clickear botón', async () => {
    const user = userEvent.setup()
    const getChainableMock = () => ({
      select: vi.fn().mockReturnThis(),
      eq: vi.fn().mockReturnThis(),
    ...
    const studentBtns = screen.queryAllByText(/estudiante/)
    if (studentBtns.length > 0) {
      await user.click(studentBtns[0])

      const modal = screen.queryByText(/Tus estudiantes/)
      if (modal) {
  ...
  it('cierra modal de estudiantes al clickear Cerrar', async () => {
    const user = userEvent.setup()
    const getChainableMock = () => ({
      select: vi.fn().mockReturnThis(),
      eq: vi.fn().mockReturnThis(),
